...
public class SaslClientAuthenticator implements Authenticator {
     ...
     * starting with {@link #SEND_APIVERSIONS_REQUEST} and ending in either {@link #COMPLETE} or
     * {@link #FAILED}.
     *
     * <p>Re-authentication of a channel starts with the state {@link
     * #REAUTH_PROCESS_ORIG_APIVERSIONS_RESPONSE} and then flows to {@link
     * #REAUTH_SEND_HANDSHAKE_REQUEST} followed by {@link
     * #REAUTH_RECEIVE_HANDSHAKE_OR_OTHER_RESPONSE} and then {@link #REAUTH_INITIAL}; after that the
     * flow joins the authentication flow at the {@link #INTERMEDIATE} state and ends at either
     * {@link #COMPLETE} or {@link #FAILED}.
     */
    public enum SaslState {
        SEND_APIVERSIONS_REQUEST, // Initial state for authentication: client sends
        // ApiVersionsRequest in this state when authenticating
        ...
        RECEIVE_HANDSHAKE_RESPONSE, // Awaiting SaslHandshake response from server when
        // authenticating
        INITIAL, // Initial authentication state starting SASL token exchange for configured
        // mechanism, send first token
        INTERMEDIATE, // Intermediate state during SASL token exchange, process challenges and send
        // responses
        CLIENT_COMPLETE, // Sent response to last challenge. If using SaslAuthenticate, wait for
        // authentication status from server, else COMPLETE
        COMPLETE, // Authentication sequence complete. If using SaslAuthenticate, this state implies
        // successful authentication.
        FAILED, // Failed authentication due to an error at some stage
        REAUTH_PROCESS_ORIG_APIVERSIONS_RESPONSE, // Initial state for re-authentication: process
        // ApiVersionsResponse from original
        // authentication
        REAUTH_SEND_HANDSHAKE_REQUEST, // Processed original ApiVersionsResponse, send SaslHandshake
        // request as part of re-authentication
        REAUTH_RECEIVE_HANDSHAKE_OR_OTHER_RESPONSE, // Awaiting SaslHandshake response from server
        // when re-authenticating, and may receive
        // other, in-flight responses sent prior to
        // start of re-authentication as well
        REAUTH_INITIAL, // Initial re-authentication state starting SASL token exchange for
        // configured mechanism, send first token
    }
     ...
     *
     * <p>Noted: there is a story about reserved range. The response of LIST_OFFSET is compatible to
     * response of SASL_HANDSHAKE. Hence, we could miss the schema error when using schema of
     * SASL_HANDSHAKE to parse response of LIST_OFFSET. For example: the IllegalStateException
     * caused by mismatched correlation id is thrown if following steps happens. 1) sent LIST_OFFSET
     * 2) sent SASL_HANDSHAKE 3) receive response of LIST_OFFSET 4) succeed to use schema of
     * SASL_HANDSHAKE to parse response of LIST_OFFSET 5) throw IllegalStateException due to
     * mismatched correlation id As a simple approach, we force Sasl requests to use a reserved
     * correlation id which is separated from those used in NetworkClient for Kafka requests. Hence,
     * we can guarantee that every SASL request will throw SchemaException due to correlation id
     * mismatch during reauthentication
     */
    ...
    public SaslClientAuthenticator(
        ...
        try {
            ...
            // the SaslClient.
            // For other mechanisms, the authenticated principal (username for PLAIN and SCRAM) is
            // used as
            ...
            else this.clientPrincipalName = null;
    ...
    SaslClient createSaslClient() {
        try {
            String kerberosDomainName = (String) configs.get("kerberos.domain.name");
            final String serviceName = Optional.ofNullable(kerberosDomainName).orElse(host);
            return Subject.doAs(
                    subject,
     ...
     *
     * <p>The messages are sent and received as size delimited bytes that consists of a 4 byte
        ...
        switch (saslState) {
            case SEND_APIVERSIONS_REQUEST:
                ...
                send(
                        apiVersionsRequest.toSend(
                                nextRequestHeader(
                                        ApiKeys.API_VERSIONS, apiVersionsRequest.version())));
            ...
            case RECEIVE_HANDSHAKE_RESPONSE:
                SaslHandshakeResponse handshakeResponse =
                        (SaslHandshakeResponse) receiveKafkaResponse();
            ...
            case REAUTH_PROCESS_ORIG_APIVERSIONS_RESPONSE:
                setSaslAuthenticateAndHandshakeVersions(
                        reauthInfo.apiVersionsResponseFromOriginalAuthentication);
            ...
            case INTERMEDIATE:
                ...
                // after a token is sent to server.
                // For versions with SASL_AUTHENTICATE header, server always sends a response to
                // each SASL_AUTHENTICATE request.
                if (saslClient.isComplete()) {
                    if (saslAuthenticateVersion == DISABLE_KAFKA_SASL_AUTHENTICATE_HEADER
                            || noResponsesPending) setSaslState(SaslState.COMPLETE);
    ...
    public void reauthenticate(ReauthenticationContext reauthenticationContext) throws IOException {
        SaslClientAuthenticator previousSaslClientAuthenticator =
                (SaslClientAuthenticator)
                        Objects.requireNonNull(reauthenticationContext).previousAuthenticator();
        ApiVersionsResponse apiVersionsResponseFromOriginalAuthentication =
    ...
    // Visible to override for testing
    protected void setSaslAuthenticateAndHandshakeVersions(
            ApiVersionsResponse apiVersionsResponse) {
        ApiVersion authenticateVersion =
                apiVersionsResponse.apiVersion(ApiKeys.SASL_AUTHENTICATE.id);
        if (authenticateVersion != null) {
            this.saslAuthenticateVersion =
                    (short)
                            Math.min(
                                    authenticateVersion.maxVersion(),
    ...
    /**
     * Sends a SASL client token to server if required. This may be an initial token to start SASL
     * token exchange or response to a challenge from the server.
     *
        ...
        if (!saslClient.isComplete()) {
            ...
            if (saslToken != null) {
                ...
                if (saslAuthenticateVersion == DISABLE_KAFKA_SASL_AUTHENTICATE_HEADER) {
                    ...
                    SaslAuthenticateRequestData data =
                            new SaslAuthenticateRequestData().setAuthBytes(tokenBuf.array());
                    SaslAuthenticateRequest request =
                            new SaslAuthenticateRequest.Builder(data)
                                    .build(saslAuthenticateVersion);
                    send =
                            request.toSend(
                                    nextRequestHeader(
                                            ApiKeys.SASL_AUTHENTICATE, saslAuthenticateVersion));
                }
                send(send);
                return true;
            }
    ...
    private byte[] createSaslToken(final byte[] saslToken, boolean isInitial) throws SaslException {
        ...
        try {
            if (isInitial && !saslClient.hasInitialResponse()) return saslToken;
            else
                return Subject.doAs(
                        subject,
            ...
            // Try to provide hints to use about what went wrong so they can fix their
            // configuration.
            if (kerberosError == KerberosError.SERVER_NOT_FOUND) {
                error +=
                        " This may be caused by Java's being unable to resolve the Kafka Broker's"
            ...
            // Treat transient Kerberos errors as non-fatal SaslExceptions that are processed as I/O
            // exceptions
            ...
            if (responseBytes == null) return null;
                ...
                AbstractResponse response =
                        NetworkClient.parseResponse(
                                ByteBuffer.wrap(responseBytes), currentRequestHeader);
     ...
     *
     * @throws KafkaException if there are no Principals in the Subject. During Kerberos re-login,
     *     principal is reset on Subject. An exception is thrown so that the connection is retried
     *     after any configured backoff.
     */
    public static String firstPrincipal(Subject subject) {
        Set<Principal> principals = subject.getPrincipals();
        synchronized (principals) {
            Iterator<Principal> iterator = principals.iterator();
            if (iterator.hasNext()) return iterator.next().getName();
            else
    ...
    private class ReauthInfo {
        ...
        public void reauthenticating(
                ApiVersionsResponse apiVersionsResponseFromOriginalAuthentication,
                long reauthenticationBeginNanos) {
            this.apiVersionsResponseFromOriginalAuthentication =
                    Objects.requireNonNull(apiVersionsResponseFromOriginalAuthentication);
         ...
         * during re-authentication that is unrelated to re-authentication, if any. This corresponds
         * to a request sent prior to the beginning of re-authentication; the request was made when
         * the channel was successfully authenticated, and the response arrived during the
         ...
         */
        public Optional<NetworkReceive> pollResponseReceivedDuringReauthentication() {
            if (pendingAuthenticatedReceives.isEmpty()) return Optional.empty();
        ...
        public void setAuthenticationEndAndSessionReauthenticationTimes(long nowNanos) {
            ...
            if (positiveSessionLifetimeMs != null) {
                ...
                double pctWindowJitterToAvoidReauthenticationStormAcrossManyChannelsSimultaneously =
                        0.10;
                double pctToUse =
                        pctWindowFactorToTakeNetworkLatencyAndClockDriftIntoAccount
                                + RNG.nextDouble()
