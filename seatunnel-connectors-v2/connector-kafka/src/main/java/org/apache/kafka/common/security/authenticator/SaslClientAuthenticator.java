...
public class SaslClientAuthenticator implements Authenticator {
    ...
    // visible for testing
    SaslClient createSaslClient() {
        String domain = (String)this.configs.get("kerberos.domain.name");
        String serverName = Optional.ofNullable(domain).orElse(host);
        try {
            return Subject.doAs(subject, (PrivilegedExceptionAction<SaslClient>) () -> {
                String[] mechs = {mechanism};
                log.debug("Creating SaslClient: client={};service={};serviceHostname={};domain={};mechs={}",
                        clientPrincipalName, servicePrincipal, host, domain, Arrays.toString(mechs));
                SaslClient retvalSaslClient = Sasl.createSaslClient(mechs, clientPrincipalName, servicePrincipal, serverName, configs, callbackHandler);
